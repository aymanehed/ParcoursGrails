...
<body>
<a href="#edit-parcours" class="skip" tabindex="-1"><g:message code="default.link.skip.label"
...
<div class="d-flex">
    <g:form controller="parcours" action="update" method="POST" enctype="multipart/form-data">
            ...
            <div style="margin: 30px "><span class="lname">Author</span>
                <select name="author.id" required="" id="author1">
                    <g:each in="${userList}" var="u">
                        <option value="${u.id}"
                                <g:if test="${u.id == parcours?.author?.id}">selected</g:if>>${u.username}</option>
                    </g:each>
                </select></div>
        </g:if>
        <div class="image-upload-container">
            ...
            <div>
                <g:each in="${parcours.illustrationList}" var="ill">
                    <asset:image src="${ill.name}"/>
                </g:each></div>
                <div class="file-drop-area">
                    <span class="file-label">Drop file here or click to upload</span>
                    <input type="file" name="illustration" class="drop-zone__input"/>
                </div>
